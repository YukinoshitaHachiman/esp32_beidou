...
void gps_publish_task(void *pvParameters) {
    while(1) {
        if(s_is_mqtt_connected) {
            gps_data_t* gps_data = get_gps_data();
...
void app_main(void) {
    ...
    // 创建数据发布任务
    xTaskCreate(gps_publish_task, "gps_publish_task", 4096, NULL, 5, &gps_publish_task_handle);
    //xTaskCreate(gyro_publish_task, "gyro_publish_task", 4096, NULL, 5, &gyro_publish_task_handle);
    ...
    while(1) {
        ...
        //ESP_LOGI(TAG, "gyr_x = %.1d  gyr_y = %.1d gyr_z = %.1d ",QMI8658C.gyr_x, QMI8658C.gyr_y, QMI8658C.gyr_z);
        if (abs(QMI8658C.gyr_x)>10000 || abs(QMI8658C.gyr_y)>10000 || abs(QMI8658C.gyr_z) > 10000) {
             esp_mqtt_client_publish(s_mqtt_client, "/warmming", "1", 0, 1, 0);
